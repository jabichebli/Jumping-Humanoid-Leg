% simulate_leg_flipping_demo.m
function simulate_leg_flipping_demo()
    % -------------------------------------------------------------------------
    ...
    params.T_hold         = 0.6;
    params.vf_takeoff     = 0.6;
    params.T_dip_ratio    = 0.75;  % 75% of jump time spent on dip/lean

    % --- FLIP PARAMETER ---
    params.x_squat_lean = 0.0; % 0.02     % The lean at the BOTTOM of the squat
    params.x_takeoff = 0.0; % 0.06 % The lean at takeoff
    ...
    params.flight_Kp = [50; 50]; % OLD: [2; 2] - Give it more power
    params.flight_Kd = [5; 5]; % OLD: [2; 2]
    params.flight_u_max = [2.0; 2.0]; % OLD: [0.2; 0.2]

    % Tuck during Flight parameter
    params.q_tuck_des = [deg2rad(200); deg2rad(160)]; % the desired "tucked" joint angles for flight [q1; q2]
    ...
    opts_stance = odeset('RelTol',1e-6,'AbsTol',1e-6, 'Events', @(t,x) event_takeoff3(t,x,params));
    opts_flight = odeset('RelTol',1e-6,'AbsTol',1e-6, 'Events', @(t,x) event_touchdown3(t,x,params), 'MaxStep', 1e-2);
    
    state = 1; % 1 = Stance (Flipping), 2 = Flight, 3 = Stance (Landing)
    ...
    while t_start < t_end
        switch state
            case 1 % Stance phase, executing the flip
                ...
                [t, X, ~, ~, ~] = ode45(@(t,x) dynamics_stance3(t,x,params), [t_start t_end], x_current, opts_stance);
                ...
                for i = 1:length(t)
                    [~, u_i, lambda_i, p_d] = dynamics_stance3(t(i), X(i,:)', params);
                    u_temp(i,:) = u_i'; 
            ...
            case 2 % Flight phase
                ...
                [t, X, ~, ~, ~] = ode45(@(t,x) dynamics_flight3(t,x,params), [t_start t_end], x_current, opts_flight);
                
                u_temp = zeros(length(t), 2);
                for i = 1:length(t); [~, u_i] = dynamics_flight3(t(i), X(i,:)', params); u_temp(i,:) = u_i'; end
            ...
            case 3 % Stance phase, stabilizing after landing
                ...
                [t, X] = ode45(@(t,x) dynamics_stabilize3(t,x,params), [t_start t_integrate_end], x_current);
                
                u_temp = zeros(length(t), 2); lambda_temp = zeros(length(t), 2);
                for i = 1:length(t); [~, u_i, lambda_i] = dynamics_stabilize3(t(i), X(i,:)', params); u_temp(i,:) = u_i'; lambda_temp(i,:) = lambda_i'; end
        ...
        if state == 2 
            ...
            x_current = dynamics_impact3(x_current, params);
        end
    ...
    animate_flipping_gif(t_all, X_all, params, 'leg_flip_animation_attempt2.gif');
